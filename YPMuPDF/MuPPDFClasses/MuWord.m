...
+ (MuWord *) word
{
    return [[MuWord alloc] init];
}

- (void) appendChar:(unichar)c withRect:(CGRect)_rect
{
    NSString *str = [NSString stringWithFormat:@"%C",c];
    NSString *string = [self.string stringByAppendingString:str];
    self.string = string;
//    [self.string stringByAppendingString:str];
//    [self.string appendFormat:@"%C", c];
    self.rect = CGRectUnion(self.rect, _rect);
}
